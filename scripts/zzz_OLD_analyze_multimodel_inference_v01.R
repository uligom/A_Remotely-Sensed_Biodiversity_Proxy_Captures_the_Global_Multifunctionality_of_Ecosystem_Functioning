...
vers <- paste0("v01.", stringr::str_extract(dat_in, "[:digit:]+[:punct:]?[:digit:]*[:punct:]?[:digit:]*"))
...
for (eee in 1:2) {
  ...
  dat <- dat %>% 
    ...
    dplyr::select(-contains(c("CLAY", "SAND", "SILT", "AWCh", "ORCDRC", "PHIHOX"))) %>% # exclude soil predictors (from modelled grid product)
    # dplyr::select(-Temp, -Precip, -SWin, -VPD) %>% # test without mean climatic variables (v04)
    # dplyr::select(-LAImax_modis) # exclude LAImax_modis for testing differences (v0Xb)
    dplyr::select(-LAImax) %>% rename(LAImax = LAImax_modis) #%>% # exclude LAImax (since LAImax_modis is more thourough) (v0X)
  # left_join(read_csv("data/inter/raoq/raoQ_S2_L2A_filtered_averaged.csv", show_col_types = F) %>% # add NIRv (v02)
    ...
    if (eee == 1) {
      ...
      if (savedata) {
        cat(
          ...
          file = "results/multimodel_inference/avg_relaimpo_RaoQ_{vers_out}.txt"
        )
      }
    }
